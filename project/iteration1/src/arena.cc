...
void Arena::UpdateEntitiesTimestep(void) {
  ...
  for (size_t i = 1; i < obstacles_.size(); i++) {
    CheckForEntityCollision(robot_, obstacles_[i], &ec,
...
void Arena::CheckForEntityCollision(const ArenaMobileEntity* const ent1,
  ...
  Position ent1_pos = Position(ent1_x, ent1_y);
  double ent2_x = ent2->get_pos().x;
  double ent2_y = ent2->get_pos().y;
  double delta_x = ent1_x - ent2_x;
  double delta_y = ent1_y - ent2_y;
  double angle_delta = atan(delta_y / delta_x);
  ...
  if (dist > ent1->radius() + ent2->radius() + collision_delta) {
    ...
    event->collided(true);
    event->point_of_contact(ent1_pos);
    event->angle_of_contact(180 - 2 * angle_delta + ent1->heading_angle());
    /// >>>>>>> FILL THIS IN
  }
